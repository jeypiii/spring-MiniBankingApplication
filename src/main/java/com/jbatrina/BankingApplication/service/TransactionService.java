...
public class TransactionService {
    ...
    public Transaction addTransaction(Transaction transaction) {
        ...
        if (transaction.getSourceAccount().isClosed()) {
            throw new AccountIsClosedException(transaction.getSourceAccount().getAccountId()).setContextMessage(
            		"Source account has already been closed", "::TRANSACTION_CLOSED_SOURCE");
        	
        }
 
        if (transaction.getTargetAccount().isClosed()) {
            throw new AccountIsClosedException(transaction.getTargetAccount().getAccountId()).setContextMessage(
            		"Target account has already been closed", "::TRANSACTION_CLOSED_TARGET");
    ...
    public Transaction closeTransaction(Transaction transaction) {
        // NOTE: For now, transactions are closed the same time they are created so closeTransaction essentially acts as addTransaction.
